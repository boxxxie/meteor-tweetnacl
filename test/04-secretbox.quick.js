Tinytest.add('nacl - secretbox and - nacl.secretbox.open', function(t) {
...
Tinytest.add('nacl - secretbox.open - with invalid box', function(t) {
  var key = new Uint8Array(nacl.secretbox.keyLength);
  var nonce = new Uint8Array(nacl.secretbox.nonceLength);
...
Tinytest.add('nacl - secretbox.open - with invalid nonce', function(t) {
  var key = new Uint8Array(nacl.secretbox.keyLength);
  var nonce = new Uint8Array(nacl.secretbox.nonceLength);
...
Tinytest.add('nacl - secretbox.open - with invalid key', function(t) {
  var key = new Uint8Array(nacl.secretbox.keyLength);
  for (var i = 0; i < key.length; i++){ key[i] = i & 0xff;}
  var nonce = new Uint8Array(nacl.secretbox.nonceLength);
  var msg = nacl.util.decodeUTF8('message to encrypt');
